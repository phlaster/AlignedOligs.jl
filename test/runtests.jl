...
@testset verbose=true "AlignedOligs.jl"  begin
    ...
    # @testset "AbstractOlig subtypes" begin
    #     # Test constructors with different input types
    ...
    #         # String constructors with valid/invalid chars
    #         valid_seqs = ["ACGT", "ACGTM", "ACGT-", "N-N-N"]
    #         constructors = [Olig, DegenOlig, GappedOlig]
            
    #         for (i, seq) in enumerate(valid_seqs)
    #             for (j, T) in enumerate(constructors)
    #                 # Skip combinations that should fail
    #                 skip = (i == 2 && j == 1) || (i == 3 && j == 1) || (i == 4 && j == 1) || (i == 3 && j == 2)
    #                 if skip
    #                     @test_throws ErrorException T(seq)
    #                 else
    #                     olig = T(seq)
    #                     @test String(olig) == seq
    #                     @test olig isa T
    #                 end
    #             end
    #         end
    ...
    #             olig = T("ACGT", desc)
    #             @test description(olig) == desc
                
    #             # Test empty description fallback
    #             olig2 = T(olig)
    #             @test description(olig2) == desc
    #         end
    #     end
        
    #     @testset "Base methods" begin
    #         # Length and indexing methods
    #         for len in [0, 1, 5, 10]
    #             for T in [Olig, DegenOlig, GappedOlig]
    #                 bases = T == Olig ? non_degen_bases : T == DegenOlig ? degen_bases : all_bases
    #                 seq = rand_seq(len, bases)
    #                 olig = T(seq)
    ...
    #                 if len > 0
    #                     @test olig[1] == seq[1]
    #                     @test olig[end] == seq[end]
                        
    #                     # Slicing
    #                     if len > 1
    #                         @test String(olig[1:2]) == seq[1:2]
    #                         @test olig[1:2] isa OligView
    #                     end
    ...
    #         # Test gap detection
    #         @test !hasgaps(Olig("ACGT"))
    #         @test !hasgaps(DegenOlig("ACGTM"))
    #         @test hasgaps(GappedOlig("A-CGT"))
    #         @test hasgaps(GappedOlig("ACGT-"))
            
    #         # Test degenerate position counting
    #         @test n_deg_pos(Olig("ACGT")) == 0
    #         @test n_deg_pos(DegenOlig("ACGTM")) == 1
    #         @test n_deg_pos(DegenOlig("MRWSYKVHDBN")) == 11
            
    #         # Test unique oligomer counting
    #         @test n_unique_oligs(Olig("ACGT")) == 1
    #         @test n_unique_oligs(DegenOlig("N")) == 4
    #         @test n_unique_oligs(DegenOlig("NN")) == 16
    #         @test n_unique_oligs(GappedOlig("-N-")) == 4
            
    #         # Test parent relationships
    #         gapped = GappedOlig("AC-GT")
    #         @test parent(gapped) isa DegenOlig
    #         @test String(parent(gapped)) == "ACGT"
            
    #         view = gapped[2:4]
    #         @test parent(view) == gapped
    #         @test olig_range(view) == 2:4
    #         @test String(view) == "C-G"
    #     end
    ...
    #             # Test empty sequence error
    #             empty_olig = T()
    #             @test_throws ArgumentError sampleChar(empty_olig)
    #         end
    ...
    #             @test_throws ArgumentError sampleView(olig, 0)
    #             @test_throws ArgumentError sampleView(olig, len+1)
    #         end
            
    #         # Test sampleNondeg
    #         degen_seq = "ACGTMRWSYKVHDBN"
    #         degen = DegenOlig(degen_seq)
    #         nondeg = sampleNondeg(degen)
    #         @test nondeg isa DegenOlig
    #         @test length(nondeg) == length(degen)
    #         @test all(c in non_degen_bases for c in String(nondeg))
            
    #         # Gapped olig sampling
    #         gapped_seq = "AC-GT-M"
    #         gapped = GappedOlig(gapped_seq)
    #         nondeg_gapped = sampleNondeg(gapped)
    #         @test nondeg_gapped isa GappedOlig
    #         @test count(==('-'), String(nondeg_gapped)) == count(==('-'), gapped_seq)
    #     end
        
    #     @testset "Type conversions and promotion" begin
    #         # Test conversion between types
    #         seq = "ACGT"
    #         degen_seq = "ACGTM"
            
    #         olig = Olig(seq)
    #         @test convert(DegenOlig, olig) isa DegenOlig
    #         @test convert(GappedOlig, olig) isa GappedOlig
            
    #         degen_olig = DegenOlig(degen_seq)
    #         @test convert(Olig, degen_olig) isa Olig  # This should fail, but let's test it
    #         @test convert(GappedOlig, degen_olig) isa GappedOlig
    ...
    #         # Test non-deterministic iterator
    #         degen = DegenOlig("NN")
    #         nondeg_iter = nondegens(degen)
    #         @test length(nondeg_iter) == 16
    #         count = 0
    #         for olig in nondeg_iter
    #             count += 1
    #             @test olig isa Olig
    #             @test length(olig) == 2
    #             @test all(c in non_degen_bases for c in String(olig))
    #         end
    #         @test count == 16
    #     end
    ...
    #             @test occursin(nameof(T), str)
    #         end
            
    #         # Test abbreviated displays for long sequences
    #         long_seq = rand_seq(100)
    #         long_olig = Olig(long_seq)
    #         io = IOBuffer()
    #         show(IOContext(io, :limit => true), long_olig)
    #         str = String(take!(io))
    #         @test length(str) < 50  # Should be abbreviated
    #     end
    # end
end